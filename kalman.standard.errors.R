...
kalman.standard.errors <- function(T, states, theta, y.data, x.data, stage,
    ...
    stin <- states$smoothed$xi.tT[1,] 
    pp1  <- states$filtered$P.ttm1[1:n.state.vars,] 
    eigenstuff.pp1   <- eigen(pp1)
    eigenvectors.pp1 <- eigenstuff.pp1$vectors
    for (l in 1:n.state.vars) {
        if (eigenvectors.pp1[1,l] < 0 ) { eigenvectors.pp1[,l] <- -eigenvectors.pp1[,l] }
    } 
    eigenvalues.pp1  <- eigenstuff.pp1$value
    ...
    info <- solve(t(likelihood.gradient) %*% likelihood.gradient) 
    bse <- sqrt(diag(info))
    ...
    eigenvectors.info <- eigenstuff.info$vectors 
    
    for (l in 1:n.params) {
        if (eigenvectors.info[1,l] < 0 ) { eigenvectors.info[,l] <- -eigenvectors.info[,l] }
    }
    eigenvalues.info  <- eigenstuff.info$value
